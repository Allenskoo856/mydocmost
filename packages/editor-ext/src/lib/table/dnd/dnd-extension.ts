...
class TableDragHandlePluginSpec implements PluginSpec<void> {
  ...
  private _pointerOverFrame?: number;
  private _pendingPointerOver?: { view: EditorView; event: PointerEvent };
  ...
  destroy = () => {
    if (this._pointerOverFrame !== undefined) {
      cancelAnimationFrame(this._pointerOverFrame);
      this._pointerOverFrame = undefined;
    }
  ...
  private _pointerOver = (view: EditorView, event: PointerEvent) => {
    this._pendingPointerOver = { view, event };
    if (this._pointerOverFrame !== undefined) return;

    this._pointerOverFrame = requestAnimationFrame(() => {
      this._pointerOverFrame = undefined;
      const pending = this._pendingPointerOver;
      this._pendingPointerOver = undefined;
      if (pending) {
        this._updatePointerOver(pending.view, pending.event);
      }
    });
  };

  private _updatePointerOver = (view: EditorView, event: PointerEvent) => {
